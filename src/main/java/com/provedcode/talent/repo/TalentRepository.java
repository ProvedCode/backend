...
//    Page<Talent> findBySkills_SkillContainsIgnoreCase(Pageable pageable, String... skill);
    ...
    @Query("SELECT DISTINCT t FROM Talent t LEFT JOIN t.skills skills WHERE UPPER(skills.skill) IN (:skills)") //if only contains all
    Page<Talent> findBySkills_SkillsInIgnoreCase(Pageable pageable, @Param("skills") List<String> skills);
